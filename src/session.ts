...
export const DEFAULT_TRANSCRIPT_DIR = join(
  homedir(),
  ".daemon-engine",
  "sessions"
);
...
export function validateSessionKey(key: string): boolean {
  ...
  if (!parts[1]) return false; // agentId must not be empty
  return true;
}
...
export function createSession(
  sessionKey: string,
  metadata: { model: string; channel: string }
): Session {
  if (!validateSessionKey(sessionKey)) {
    throw new Error(
      `Invalid session key format. Expected agent:{agentId}:{rest}, got: ${sessionKey}`
    );
  }
...
export async function persistSession(
  session: Session,
  transcriptDir: string = DEFAULT_TRANSCRIPT_DIR
): Promise<void> {
